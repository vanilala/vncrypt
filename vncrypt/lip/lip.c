	    ...
	    )
{
	...
	/* store the length (which includes integer sign) */
	bitstring[0] = len_bitstring&((1<<CHARL)-1);
	bitstring[1] = (len_bitstring>>CHARL)&((1<<CHARL)-1);
	bitstring[2] = (len_bitstring>>(2*CHARL))&((1<<CHARL)-1);
	bitstring[3] = (len_bitstring>>(3*CHARL))&((1<<CHARL)-1);

	if (len_bitstring < 0)
		len_bitstring = -len_bitstring;
	lbi = 0; hbi = CHARL-1;
	for (i=0; i < len_bitstring; ++i) {
	    int   lbw /* low bit word */, hbw /* high ... */;
	    int   lbwi /* low bit word index */, hbwi /* high ... */;
	    long  bits;

	    /* get bits indexed by  lbi, ..., hbi  which is the same as
	       bits  i*CHARL, ... , (i+1)*CHARL - 1  */
	    lbw = (lbi / NBITS) + 1;
	    hbw = (hbi / NBITS) + 1;
	    lbwi = lbi % NBITS;
	    if (lbw == hbw) {
		/* can get the bits entirely within one word */
		bits =  (a[lbw] >> lbwi) & ((1 << CHARL)-1);
	    }
	    else {
		/* get the high  NBITS - lbwi  bits of word indexed
		   by  lbw . */
		bits = a[lbw] >> lbwi;
		hbwi = hbi % NBITS;
     
		if (hbw <= len_a) {
		    /* the remaining  CHARL - (NBITS - lbwi) bits are at
		       the start of the next word */
		    bits |= ( a[hbw] & ((1 << (hbwi+1)) -1) ) <<
			    (CHARL - 1 - hbwi);
		}

	    }

	    bitstring[4 + i] = (char)bits;
	    lbi += CHARL;
	    hbi += CHARL;
	} /* for (i=0; i < len_bitstring; ++i) */
...
{
    int len_string, len_a;
    int anegative = 0;
    verylong a = *aa;
    register int i;
    int bit_num;

    len_string = ((int)string[0]) + ((int)string[1]<<8) + ((int)string[2]<<16)
      + ((int)string[3]<<24);
    if (len_string < 0) {
	len_string = -len_string;
	anegative = 1;
    }
    len_a = 1+(len_string*CHARL-1)/NBITS;

    zsetlength( &a, len_a, "in zbitstringtoz, second argument" );
    a[len_a] = 0;
	...
	/*  put  string[i+4]  into location corresponding to bits
	    CHARL*i,...,CHARL*(i+1)-1. */
	wordloc = bit_num/NBITS + 1;
	shift = bit_num % NBITS;
	if (shift == 0)
	    a[wordloc] = (long)string[i+4];
	else if (shift <= NBITS-CHARL)
	    a[wordloc] |= (long)string[i+4] << shift;
	else {
	    a[wordloc] |= ((long)string[i+4] << shift) & RADIXM;
	    a[wordloc+1] = (long)string[i+4] >> (NBITS-shift);
	}
	...
	)
{
	...
	string = ztobitstring( a );

	/*  len  holds the length and sign */
	len = ((int)string[0]) + ((int)string[1]<<8) + ((int)string[2]<<16)
	  + ((int)string[3]<<24);
	i = htonl(len);
	if (fwrite((void*)(&i), sizeof(int), (size_t)1, fn ) < 1) {
	    free(string);
	    return 0;
	}
	if (len < 0) len = -len;
	for (j=0;j<len;j++) {
		if (fwrite((void*)(&string[j+4]), 1, (size_t)1, fn) < 1) {
			free(string);
			return (0);
		}
	}
	...
	)
{
	...
	if (feof(fn) || ferror(fn))
		return (0);
	if ((int)fread((void*)&len, sizeof(int), 1, fn) < 1)
		return (0);
	len = ntohl(len);
	string = (unsigned char *) malloc( abs(len)+4 );
	string[0] = len&((1<<CHARL)-1);
	...
	)
{
	...
	bl = (b%NBITS);
	if (bl) (*c)[wh] &= ((1L<<bl)-1); /* AKL 19981006 */
	while ((wh>1) && (!((*c)[wh])))
		wh --;
	(*c)[0] = wh;
}
	...
	)
{
	...
	{
		zhalt("nth root with n=0 in zroot");
		FREESPACE(a);
		return -1;
	}
	else if ((signa < 0) && (!(n & 1)))
	{
		zhalt("nth root with even n of negative number in zroot");
		FREESPACE(a);
		return -1;
	}
	else if (signa == 0 && n < 0)
	{
		zhalt("nth root with n<0 of zero in zroot");
		FREESPACE(a);
		return -1;
	}
	else
...
{
	return zfecm(n,f,uni,&nb,&bound,grow,co,ecmonly,info,stdout);
}
	...
	)
{
 ...
 /* grow is growth of bound per iteration */
 ...
 /* |info|>1: timing per phase and per curve */
	...
	for (i = *nb; i > 0; i--)
	{
		...
		accumulated_growth *= grow;
		*bound = (long) (0.5 + orig_bound * accumulated_growth);
